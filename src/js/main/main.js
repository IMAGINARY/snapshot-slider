...
const argv = require('yargs')
    .command('$0', 'the default command', {}, ui)
    .command('update', 'Update the list of SNAPSHOTS', {}, updateSnapshotList)
    .command('download', 'Download missing SNAPSHOT PDFs', {}, download)
    .command('preprocess', 'Pre-render the pages of the PDFs', {}, preprocess).argv;
...
async function updateSnapshotList() {
    ...
    try {
        ...
        if (_.isEqual(newSnapshotList.snapshots, miscConfig.snapshots.articles)) {
            console.log(`${successNotificationTitle}: No change detected.`);
        } else {
            settings.set("snapshots.articles", newSnapshotList.snapshots, {
                prettify: true
            });
            console.log(successNotificationTitle)
        }
        app.exit();
    } catch (error) {
        switch (error.errorCode) {
            case SnapshotListUpdater.UpdateError.ERROR_DOWNLOAD_FAILED:
                console.error(errorNotificationTitle, "The list could not be downloaded.", error.cause);
                break;
            case SnapshotListUpdater.UpdateError.ERROR_VERSION_MISMATCH:
                console.error(errorNotificationTitle, "Online data has wrong version tag.", error.cause);
                break;
            case SnapshotListUpdater.UpdateError.ERROR_VALIDATION_FAILED:
                console.error(errorNotificationTitle, "The downloaded list is invalid.", error.cause);
                break;
        }
        app.exit(-1);
    }
